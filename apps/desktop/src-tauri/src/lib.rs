...
fn set_window_tab_state(
    ...
    state
        .has_tabs_by_window
        .lock()
        .expect("window document state poisoned")
        .insert(window.label().to_string(), has_tabs);
}
...
fn close_last_tab_or_quit_app(
    ...
    let open_window_labels = app.webview_windows().into_keys().collect::<HashSet<_>>();
    let action = {
        let mut has_tabs_by_window = state
            .has_tabs_by_window
            .lock()
            .expect("window document state poisoned");

        last_tab_close_action(&current_label, &open_window_labels, &mut has_tabs_by_window)
    };

    match action {
        LastTabCloseAction::CloseWindow => window
            .close()
            .map_err(|err| format!("Unable to close empty window: {err}")),
        LastTabCloseAction::QuitApp => {
            app.exit(0);
...
fn open_sosein_workspace_window(app: AppHandle) -> Result<(), String> {
    ...
    let builder = WebviewWindowBuilder::new(
        &app,
        SOSEIN_WORKSPACE_WINDOW_LABEL,
        WebviewUrl::App("index.html?workspace=sosein".into()),
    )
    .title("Margin")
...
#[cfg(target_os = "ios")]
#[tauri::command]
fn open_sosein_workspace_window() -> Result<(), String> {
    Err("Sosein Cloud workspace windows are not available on iOS.".to_string())
}
...
mod native_document_tests {
    ...
    #[test]
    fn closing_last_tab_closes_only_the_empty_window_when_another_window_has_tabs() {
        let open_window_labels = HashSet::from(["main".to_string(), "sosein-cloud".to_string()]);
        let mut has_tabs_by_window = HashMap::from([
            ("main".to_string(), true),
            ("sosein-cloud".to_string(), true),
        ]);

        let action =
            last_tab_close_action("sosein-cloud", &open_window_labels, &mut has_tabs_by_window);

        assert_eq!(action, LastTabCloseAction::CloseWindow);
        assert_eq!(has_tabs_by_window.get("sosein-cloud"), Some(&false));
        assert_eq!(has_tabs_by_window.get("main"), Some(&true));
    }

    #[test]
    fn closing_last_tab_quits_when_all_open_windows_have_no_tabs() {
        let open_window_labels = HashSet::from(["main".to_string(), "sosein-cloud".to_string()]);
        let mut has_tabs_by_window = HashMap::from([
            ("main".to_string(), false),
            ("sosein-cloud".to_string(), true),
            ("closed-window".to_string(), true),
        ]);

        let action =
            last_tab_close_action("sosein-cloud", &open_window_labels, &mut has_tabs_by_window);

        assert_eq!(action, LastTabCloseAction::QuitApp);
        assert_eq!(has_tabs_by_window.get("sosein-cloud"), Some(&false));
        assert!(!has_tabs_by_window.contains_key("closed-window"));
    }

    #[test]
    fn closing_last_tab_keeps_app_open_for_an_unreported_other_window() {
        let open_window_labels = HashSet::from(["main".to_string(), "sosein-cloud".to_string()]);
        let mut has_tabs_by_window = HashMap::from([("sosein-cloud".to_string(), true)]);
...
#[cfg(not(target_os = "ios"))]
fn emit_to_focused_window<S>(app: &AppHandle, event: &str, payload: S)
where
    S: Clone + Serialize,
{
    ...
    if windows.len() == 1 {
        if let Some(window) = windows.into_values().next() {
            let _ = window.emit(event, payload);
        }
    }
}

fn last_tab_close_action(
    current_label: &str,
    open_window_labels: &HashSet<String>,
    has_tabs_by_window: &mut HashMap<String, bool>,
) -> LastTabCloseAction {
    has_tabs_by_window.retain(|label, _| open_window_labels.contains(label));
    has_tabs_by_window.insert(current_label.to_string(), false);

    let any_other_window_with_tabs = open_window_labels
        .iter()
        .filter(|label| label.as_str() != current_label)
        .any(|label| has_tabs_by_window.get(label).copied().unwrap_or(true));

    if any_other_window_with_tabs {
        LastTabCloseAction::CloseWindow
    } else {
        LastTabCloseAction::QuitApp
    }
}
...
pub fn run() {
    ...
    let builder = builder.on_menu_event(|app, event| {
        ...
        if menu_id.as_ref() == "app_settings" {
            emit_to_focused_window(app, "margin://open-settings", ());
        } else if menu_id.as_ref() == "app_check_updates" {
            emit_to_focused_window(app, "margin://check-for-updates", ());
        } else if menu_id.as_ref() == "tools_word_count" {
            emit_to_focused_window(app, "margin://show-word-count", ());
        } else if menu_id.as_ref() == "tools_install_cli" {
            show_cli_install_result(install_margin_cli());
        } else if menu_id.as_ref() == "file_new" {
            emit_to_focused_window(app, "margin://new-document", ());
        } else if menu_id.as_ref() == "file_quick_open" {
            emit_to_focused_window(app, "margin://quick-open-document", ());
        } else if menu_id.as_ref() == "file_open" {
            emit_to_focused_window(app, "margin://open-document", ());
        } else if menu_id.as_ref() == "file_open_folder" {
            emit_to_focused_window(app, "margin://open-folder", ());
        } else if let Some(index) = recent_document_menu_index(menu_id.as_ref()) {
            emit_to_focused_window(app, "margin://open-recent-document", index);
        } else if menu_id.as_ref().starts_with(CLEAR_RECENT_MENU_ITEM_PREFIX) {
            emit_to_focused_window(app, "margin://clear-recent-documents", ());
        } else if menu_id.as_ref() == "file_save" {
            emit_to_focused_window(app, "margin://save-document", ());
        } else if menu_id.as_ref() == "file_save_as" {
            emit_to_focused_window(app, "margin://save-document-as", ());
        } else if menu_id.as_ref() == "file_print" {
            emit_to_focused_window(app, "margin://print-document", ());
        } else if menu_id.as_ref() == "file_close_tab" {
            emit_to_focused_window(app, "margin://close-tab", ());
        } else if menu_id.as_ref() == "file_close_window" {
            if let Some(window) = focused_webview_window(app) {
                let _ = window.close();
            }
        } else if menu_id.as_ref() == "window_previous_tab" {
            emit_to_focused_window(app, "margin://previous-tab", ());
        } else if menu_id.as_ref() == "window_next_tab" {
            emit_to_focused_window(app, "margin://next-tab", ());
        } else if menu_id.as_ref() == "edit_command_palette" {
            emit_to_focused_window(app, "margin://open-command-palette", ());
        } else if menu_id.as_ref() == "edit_find" {
            emit_to_focused_window(app, "margin://open-find", ());
        } else if menu_id.as_ref() == "edit_find_and_replace" {
            emit_to_focused_window(app, "margin://open-find-and-replace", ());
        } else if menu_id.as_ref() == "edit_find_next" {
            emit_to_focused_window(app, "margin://find-next", ());
        } else if menu_id.as_ref() == "edit_find_previous" {
            emit_to_focused_window(app, "margin://find-previous", ());
        } else if menu_id.as_ref() == "view_toggle_file_tree" {
            emit_to_focused_window(app, "margin://toggle-file-tree", ());
        } else if matches!(menu_id.as_ref(), "insert_comment" | "context_add_comment") {
            emit_to_focused_window(app, "margin://add-comment", ());
        } else if let Some(kind) = insert_payload(menu_id.as_ref()) {
            emit_to_focused_window(app, "margin://insert-block", kind);
        }
    });
